  ...
  renderDates(dates) {
    return dates.map((date, i) => (
      <div className="flex flex-row sm:flex-col mr-1 w-full flex-wrap" key={i}>
        <div className="flex flex-col sm:flex-col mb-2 sm:w-full flex-grow w-full sm:w-auto sm:flex-no-grow text-grey-darkest">
          <span className="font-bold text-left">{date.day.toUpperCase()}</span>
    ...
    return generateBookableIntervals().map((x, i) => {
      ...
      return (
        <div
          key={i}
          onClick={
            !isReserved ? this.handleTimeslotClicked(i, dateString) : undefined
          }
